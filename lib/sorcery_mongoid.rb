  ...
  class Adapter < Sorcery::Adapters::BaseAdapter
    ...
    class << self
      ...
      def find_by_oauth_credentials(provider, uid)
        auth_klass ||= @klass.sorcery_config.authentications_class
        if auth = auth_klass.where(@klass.sorcery_config.provider_attribute_name => provider, @klass.sorcery_config.provider_uid_attribute_name => uid).first
          find_by_id(auth.send(@klass.sorcery_config.authentications_user_id_attribute_name))
        end
      end
